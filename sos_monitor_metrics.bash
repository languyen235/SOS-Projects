...
## global $CLIOSOFT_DIR variable must have been defined
...
key="${attr##*_}"  # Remove front value. Remove text from beggining up to underscore
...
# Use a lock to ensure that only one process is creating csv file at any given time.
    ...
    do
		...
		# remove  .<site>.intel.com
		phost="${phost%%.*}" && chost="${chost%%.*}"

		if [[ $ptype == LOCAL ]]; then
			echo "$service,$phost,repo,$pcport,$prpath" >> "$data_file"        # output repo server metrics to file
			echo "$service,$chost,cache,$ccport,$cpath" >> "$data_file"      # output cache server metrics to file
		else
			echo "$service,$chost,remote_cache,$ccport,$cpath" >> "$data_file"   # save remote cache server metrics to file
		fi
		unset atts
    done
...
[[ -e $infile ]] || { echo "-F: Source file not found"; exit 1; } 
	...
	if [[ "$DEBUG" == off ]]; then
		...
		# variable subtitution for .$$ to .prom extension
...
# This copying, typically on the /tmp local disks is time ~1.5s
	...
	exec 200>/tmp/monitor_nfs_times.lck || { echo "-E: Exec command failed"; return 10; }

	#Acquire a lock using file handle
	flock -n 200 || { echo "-E: There is another process updating the file"; return 10; }
	trap "flock -u 200; rm -f /tmp/monitor_nfs_times.lck" INT TERM EXIT
...
tmpfile=${tmp_dir}/Check_Cliosoft_Disk_Space.$$
    ...
    echo "# TYPE Check_Cliosoft_Disk_Space counter" >> "$tmpfile"
...
#----------
	...
	case "${1-}" in 
		...
		*)
			...
			echo "Usage:"
			echo "$0  [-d ] : Run all monitoring without creating new promp files"
